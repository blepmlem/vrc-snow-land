    ...
    void initTextureData(inout float4 albedo, inout float4 mainTexture, inout float3 backFaceEmission, inout float3 dissolveEmission, in half3 detailMask)
    {
        ...
        mainTexture.a *= alphaMask;
        ...
        #ifndef POI_SHADOW
            albedo = float4(mainTexture.rgb * max(float4(0.6817279,0.7770396,1,1).rgb, float3(0.000000001, 0.000000001, 0.000000001)) * lerp(1, GammaToLinearSpace(poiMesh.vertexColor.rgb), float(0)), mainTexture.a * max(float4(0.6817279,0.7770396,1,1).a, 0.0000001));
            ...
            applyBackFaceTexture(backFaceDetailIntensity, mixedHueShift, albedo, backFaceEmission);
                ...
                albedo.rgb = saturate(albedo + float(0) * hueShiftAlpha.g);
            #endif
            #ifdef FINALPASS
                #if defined(PROP_DETAILTEX) || !defined(OPTIMIZER_ENABLED)

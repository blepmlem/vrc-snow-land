    ...
    void applyGlitter(inout float4 albedo, inout float3 glitterEmission)
    {
        ...
        // Scale
        float2 st = poiMesh.uv[0] * float(256);
        ...
        switch(float(0))
        {
            ...
            finalGlitter = lerp(0, float(0.009) * glitterAlpha, glitterAlpha) + max(pow(saturate(dot(lerp(glitterReflectionDirection, poiCam.viewDir, float(0.617)), poiCam.viewDir)), float(300)), 0);
            finalGlitter *= glitterAlpha;
            break;
            case 1:
            float offset = random(randoPoint);
            float brightness = sin((_Time.x + offset) * float(2)) * float(20) - (float(20) - 1);
            finalGlitter = max(float(0.009) * glitterAlpha, brightness * glitterAlpha * smoothstep(0, 1, 1 - m_dist * float(0.08) * 10));
            break;
        }
        
        
        half3 glitterColor = float4(6.498019,6.498019,6.498019,1);
        glitterColor *= lerp(1, albedo, float(0.531));
        #if defined(PROP_GLITTERCOLORMAP) || !defined(OPTIMIZER_ENABLED)

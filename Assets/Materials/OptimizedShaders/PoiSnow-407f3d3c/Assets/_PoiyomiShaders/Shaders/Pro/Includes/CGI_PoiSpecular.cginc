    ...
    float3 calculateSpecular(in float4 albedo)
    {
        ...
        if(float(2) == 4)
        {
            ...
            finalSpecular += toonAnisoSpecular(float(0), float(0), poiLight.direction, poiLight.halfDir, specularMap, poiLight.nDotL, float(0.95), float(0.96), float4(1,1,1,1), albedo, metallic, jitter, float(0), packedTangentMap);
            ...
            if(float(2) == 2) // Toon
            {
                finalSpecular += calculateToonSpecular(albedo, poiMesh.uv[0], float4(0.1600193,0.2100193,0,1), metallic, float(1), specularMap, poiLight.color, poiMesh.normals[float(1)], poiLight.halfDir, poiLight.attenuation);
